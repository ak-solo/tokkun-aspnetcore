...
> **`name` 属性はプロパティ名（PascalCase）で書く**
>
> フォームの `name` 属性は ASP.NET Core のモデルバインディングがプロパティ名と照合します。
> 照合は大文字・小文字を区別しませんが、アンダースコアは無視されません。
> そのため `name="hire_date"` は `HireDate` にマッチせず、値が `null` になります。
> `name="HireDate"` または `name="hireDate"` と書いてください。
>
> なお、DB から C# へのマッピング（Dapper）では `Program.cs` の設定でスネークケースを自動変換していますが、フォームのバインディングには適用されません。
...
`type="number"` を使います。`name` 属性は問題 3-1 と同様にプロパティ名で指定します（`name="DeptId"`）。
