...
`dotnet run` でアプリを起動し、ブラウザで `/Employee/Delete/1` に直接アクセスすると削除確認画面が表示されます。
（詳細画面・一覧画面への「削除」リンクはまだありません。この章の演習で追加します。）
...
- `Views/Employee/Details.cshtml` — 詳細画面に「削除」リンクを追加する
