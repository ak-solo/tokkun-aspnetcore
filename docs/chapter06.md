...
```
/employees?keyword=田中&sortBy=salary&sortDir=desc
         ↑                ↑              ↑
...
```csharp
// /employees?keyword=田中 にアクセスすると keyword = "田中" になる
...
`dotnet run` でアプリを起動し、`/employees` にアクセスします。
...
- 検索後に URL が `/employees?keyword=田中` のようになること
...
**確認ポイント：**
- `/employees?sortBy=salary` にアクセスすると給与の高い順に並ぶこと
- `/employees?sortBy=name` にアクセスすると名前の降順に並ぶこと
- 不正な値（`/employees?sortBy=DROP TABLE` 等）では `hire_date` でソートされること
