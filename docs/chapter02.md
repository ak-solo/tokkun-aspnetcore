...
`dotnet run` でアプリを起動し、社員一覧の「詳細」リンクをクリックすると詳細画面が表示されます。
現時点では **ID** と **氏名** の 2 項目のみ表示されています。
...
この章の練習問題では、この詳細画面に項目を追加したり、部署名を JOIN で表示したりします。
...
- 問題 2-1 と同じパターンで `<dt>` / `<dd>` を追加するだけです
...
### 問題 2-3: 存在しない ID にアクセスしたときの挙動を確認する
